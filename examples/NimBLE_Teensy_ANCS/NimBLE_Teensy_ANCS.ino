...
//
// Ported to Teensy 4.1 + Murata Type 1YN. This example's loop() contains
// nested while(1)/while(incomingCall) loops that never return to the outer
// loop() while a phone is connected -- nimble_port_teensy_pump() has been
// added inside those too, since nothing else drives the BLE host on Teensy
// (no RTOS task does it for you).
#include <Arduino.h>
