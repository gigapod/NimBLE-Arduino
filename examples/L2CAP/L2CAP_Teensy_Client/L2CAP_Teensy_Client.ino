...
// it for you on Teensy). The original's blocking delay()s while waiting for
// a device/connection/channel were replaced with a small helper that keeps
// pumping while it waits, so the host doesn't stall during connection setup.
//
...
/** Wait ms milliseconds while still servicing the BLE host. */
...
void setup() {
    Serial.begin(115200);
    Serial.println("Starting L2CAP Teensy client example");
