/*
   NimBLE_Teensy_Beacon_Scanner

   Scans for iBeacon and Eddystone TLM beacons on Teensy 4.1 + Murata Type
   1YN. Pairs with NimBLE_Teensy_iBeacon / NimBLE_Teensy_EddystoneTLM_Beacon.

   Based on Neil Kolban's example for IDF, ported to Arduino ESP32 by Evandro
   Copercini, adapted for Teensy: nimble_port_teensy_pump() must be called
   every loop() iteration for the BLE host to run (no RTOS task does it for
   you on Teensy), including while blocked inside pBLEScan->getResults()
   below, which the cooperative NPL pumps internally while it waits.
*/
